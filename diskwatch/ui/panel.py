...
class DetailPanel(QWidget):
    ...
    def _on_days_ready(self, req: int, payload: object) -> None:
        ...
        self.day_box.clear()
        for d in days:
            label, tip = self._day_picker_text(d, d.day == today)
    ...
    @staticmethod
    def _day_picker_text(d, is_today: bool) -> tuple[str, str]:
        """日期下拉项的 (按钮文案, 悬浮提示)。free 为 None 表示当天无空间采样。"""
        count = f"{d.count}"
        ...
        if is_today:
            if free is not None:
                return (
                    tr("今天 · {count} 个 · {size} · 剩 {free}",
                       count=count, size=size, free=free),
                    tr("今天  ·  {count} 个  ·  {size}  ·  剩 {free}",
                       count=count, size=size, free=free),
                )
            return (
                tr("今天 · {count} 个 · {size}", count=count, size=size),
                tr("今天  ·  {count} 个  ·  {size}", count=count, size=size),
            )
        if free is not None:
            return (
                tr("{day} · {count} 个 · {size} · 剩 {free}",
                   day=d.day, count=count, size=size, free=free),
                tr("{day}  ·  {count} 个  ·  {size}  ·  剩 {free}",
                   day=d.day, count=count, size=size, free=free),
            )
        return (
            tr("{day} · {count} 个 · {size}", day=d.day, count=count, size=size),
            tr("{day}  ·  {count} 个  ·  {size}", day=d.day, count=count, size=size),
        )
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        if is_group:
            # 组行只有展开/折叠动作（"打开"切换展开状态）；文件行才有定位/复制
    ...
    def _on_day_ready(self, req: int, payload: object) -> None:
        ...
        self._apply_compiled(records, top, expand_rows)
        self.count_label.setText(
            self._status_text(len(records), count, keyword, day_total)
        )
    ...
    def _apply_compiled(
        ...
        """应用后台编译好的视图：重置模型、刷新排序指示、展开策略，并保持滚动位置。"""
    ...
    def _restore_status_from_meta(self, fallback_count: int) -> None:
        """从 _fill_meta 重算状态栏文案（排序重编译/导出完成后恢复）。"""
        meta = self._fill_meta
        if meta is None:
            return
        count = int(meta.get("count", fallback_count))
        self.count_label.setText(
            self._status_text(
                self._model.file_count(),
                count,
                str(meta.get("keyword", "")),
                int(meta.get("day_total", meta.get("count", fallback_count))),
            )
        )
    ...
    def _on_compile_ready(
        ...
        self._apply_compiled(self._model.records(), top, expand_rows)
        self._restore_status_from_meta(0)
    ...
    def _open_selected(self, index: QModelIndex | None = None) -> None:
        ...
        if self._model.data(idx, IS_GROUP_ROLE):
            # 展开状态是整行级的，但 expandedIndexes 按 (row, column) 区分，
            # 非列 0 的 index 会查不到展开状态（右键菜单路径同样要归一化）
            row_idx = idx.sibling(idx.row(), 0)
            if self.table.isExpanded(row_idx):
                self.table.collapse(row_idx)
            else:
                self.table.expand(row_idx)
            return
    ...
    def _export_csv(self) -> None:
        ...
        def work() -> None:
            ...
            def done() -> None:
                ...
                self._restore_status_from_meta(n)
